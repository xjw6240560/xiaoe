...
class Random_generate_project_type:
    ...
    bidEvaluationMethod = ['综合评标办法','均值评标办法','最大值评标办法','最小值评标办法','线下评标','单一来源','竞争性磋商','竞争性谈判']
        ...
        for i in range(num):
            ...
            if projectType == '工程':
                if role =='招标人':
                    ...
                    print(projectType+'  '+role+'  '+tenderOrganizationType+'  '+tenderWay+'  '+bidEvaluationMethod)
                    continue
                else:
                    tenderWay = self.tenderWay_random(0,1)
                    bidEvaluationMethod = self.bidEvaluationMethod_random(4)
                ...
                if role =='招标人':
                    ...
                    if tenderWay in ('竞争性磋商','竞争性谈判','单一来源'):
                        ...
                        while bidEvaluationMethod in ('竞争性磋商','竞争性谈判','单一来源') and bidEvaluationMethod != tenderWay:
                            bidEvaluationMethod = self.bidEvaluationMethod_random()
                        print(projectType+'  '+role+'  '+tenderOrganizationType+'  '+tenderWay+'  '+bidEvaluationMethod)
                        continue
                    else:
